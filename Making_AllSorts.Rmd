...
gg <- ggplot(confusion,aes(x=RF2pred,y=True,fill=Freq))  
gg + geom_tile() + labs(fill="Frequency",x="Predicted") + geom_text(aes(label=Freq),show.legend = FALSE,colour="white")
dev.off()
```
...
prob_plot = function(class_name,df){
    ...
    gg <- ggplot(dat)
    gg + geom_bar(aes(x=Sample,y=prob,fill=class),stat='identity') + theme(axis.text.x=element_blank(),axis.ticks.x=element_blank(),axis.title.y=element_text(size=18),axis.title.x=element_text(size=18)) + labs(y="Probability") 
}
